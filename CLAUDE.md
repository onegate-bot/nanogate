...
|------|------|----------|-------------|
| `src/gateway-providers/onegate.ts` | 3-way proxy switch registered as a `GatewayProvider` under kind `onegate` (`readProxyMode()` → `onegate` / `direct` / `onecli`), and the **default** provider kind (`DEFAULT_GATEWAY_PROVIDER_KIND` in `src/gateway-providers/index.ts`) | plain `onecli` provider is upstream's only provider and its default | Keep `onegate` registered and default. The `onecli` mode branch delegates to the upstream `onecli` provider (`getGatewayProviderFactory('onecli')`) rather than re-implementing it, so any upstream change to OneCLI's own contribution lands automatically — nothing to fold by hand. |
...
Since the driver-seam migration, gateway integration is a typed `GatewayProvider`
plugin (`contribute()` returns a `GatewayContribution` of `env`/`mounts`/`containers`)
rather than raw argv assembly in `container-runner.ts`; the two ordering invariants
that used to guard the old argv-based switch are now structural instead of
convention-based, and don't need separate enforcement:

- Mount shadowing is `mergeMounts()`'s job (`src/container-runner.ts`): a contributed
  mount (e.g. the OneGate CA stub) always wins a `containerPath` collision against a
  composed one, regardless of call order — no more "proxy block after mounts" rule.
- `contribute()` is invoked exactly once per session by the driver via
  `composeSessionSpec()`, so there's no analogue of the old "call `hostGatewayArgs()`
  exactly once" footgun — there is nothing to accidentally double-call.

`src/onegate-proxy.ts` holds the actual mode-read/env/CA-mount logic
(`readProxyMode`, `applyOneGateContainerConfig`, `applyDirectContainerConfig`);
`onegate.ts` is the thin `GatewayProvider` wrapper around it. Both are fork-only.

Also fork-only, but **not** at risk from upstream merges (nothing upstream touches
them): `src/onegate-proxy.ts`, `src/gateway-providers/onegate.ts`, `src/message-id.ts`,
`onegate-switch/`, `scripts/ncc`, `.claude/skills/ncc/`.
